...
// ==========================================
// 🚀 BACKEND API & CRON LOGIC (24/7 AUTO-PILOT)
// ==========================================
if (isset($_GET['action']) || isset($_POST['action'])) {
    ...
    // 🌟 24/7 SERVER AUTOPILOT (CRON JOB LOGIC) 🌟
    if ($action == 'cron_job' || $action == 'sync') {
        
        // Cron Job কল করলে সে নিজে API থেকে ডেটা নিবে
        if ($action == 'cron_job') {
            $ch = curl_init('https://api.inpay88.net/api/webapi/GetNoaverageEmerdList');
            $payload = json_encode([
                "pageSize" => 1, "pageNo" => 1, "typeId" => 30, "language" => 0, 
        ...
        if (!empty($list)) {
            // পুরানো থেকে নতুন ডেটার দিকে লুপ
            $reversed = array_reverse($list);
            
            // 1. Sync & Real Gap Filler (সারাক্ষণ রিয়েল গেম খেলার লজিক)
            foreach ($reversed as $item) {
                ...
                if (!$row) {
                    // সার্ভার কোনো কারণে মিস করলে সে রিয়েল লজিকে খেলে Win/Loss জেনারেট করবে
                    $pred = (rand(0, 100) > 50) ? 'BIG' : 'SMALL';
                    $status = 'L';
                    ...
                    // আগে থেকে প্রেডিক্ট করা রেজাল্টের সাথে আসল গেম মিলিয়ে দেখা
                    $pred = $row['prediction'];
                    $status = 'L';
                    if ($pred == 'BIG' && $number >= 5) $status = 'W';
                    elseif ($pred == 'SMALL' && $number <= 4) $status = 'W';
            ...
            // 2. Predict NEXT 3 PERIODS (অ্যাডভান্সড AI - আগে থেকেই ৩টা গেম খেলে রাখবে)
            $latestPeriod = $reversed[count($reversed)-1]['issueNumber'] ?? $reversed[count($reversed)-1]['issue'] ?? $reversed[count($reversed)-1]['period'];
            $nextP = $latestPeriod;

            for ($i = 0; $i < 3; $i++) {
                $nextP = addOneStr($nextP);
                $stmt = $pdo->prepare("SELECT prediction FROM naruto_history WHERE period = ?");
                $stmt->execute([$nextP]);
                
                if (!$stmt->fetch()) {
                    // ABAB Pattern Checking
                    ...
                    $ins->execute([$nextP, $prediction]);
                }
            }
            echo json_encode(['success' => true, 'message' => 'AI Played 24/7 Successfully!']);
        } else {
            echo json_encode(['success' => false, 'message' => 'Data Empty']);
        }
        exit;
    }
    ...
    if ($action == 'get_prediction') {
        ...
        if ($row) echo json_encode(['prediction' => $row['prediction'], 'pattern' => false]);
            ...
            // Fallback (if not generated by Cron yet)
            $prediction = (rand(0, 100) > 50) ? 'BIG' : 'SMALL';
            $ins = $pdo->prepare("INSERT IGNORE INTO naruto_history (period, prediction) VALUES (?, ?)");
            $ins->execute([$period, $prediction]);
            echo json_encode(['prediction' => $prediction, 'pattern' => false]);
        }
        exit;
    }
        ...
        async function fetchPeriod(callback) {
            try {
                let r = await fetch(API_URL, {
                    ...
                    body: JSON.stringify({ "pageSize": 1, "pageNo": 1, "typeId": 30, "language": 0, "random": "61bba", "signature": "558BB", "timestamp": 1779019083 })
                });
