...
if (isset($_GET['action']) || isset($_POST['action'])) {
    ...
    if ($action == 'get_prediction') {
        ...
        if ($row) {
            ...
            echo json_encode(['prediction' => 'WAITING', 'pattern' => false]); // Worker এখনো আপডেট না করলে
        }
        exit;
    }
...
<head>
    ...
    <title>NARUTO AI MAX - PRO PANEL</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        .color-wait { color: #555; text-shadow: none; }
        ...
        .ball-wait { color: #555; border-color: #555; box-shadow: none; }
...
<body>
    ...
    <script>
        ...
        // 🟢 Frontend ONLY asks API for Period Number (Super Fast)
        ...
        // 🟢 Frontend ONLY reads Prediction from Database
        async function generateAIResult() {
            try {
                let res = await fetch(`?action=get_prediction&period=${currentPeriod}`);
                let data = await res.json();
                if(data && data.prediction && data.prediction !== 'WAITING') { 
                    renderUI(data.prediction, data.pattern); return; 
                }
            } catch(e) {}
            
            // Worker যদি ২/৩ সেকেন্ড লেট করে, তখন Waiting দেখাবে
            document.getElementById('signal-text').innerText = "WAITING";
            document.getElementById('signal-text').className = "status color-wait";
            document.getElementById('sub-signal').innerText = "FETCHING DB...";
            document.getElementById('ball-container').innerHTML = `<div class="ball ball-wait">?</div><div class="ball ball-wait">?</div>`;
        }
        ...
        async function deleteHistory() {
            ...
            if(pass) {
                let res = await fetch('?action=delete_history', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({ password: Hack1135hosT }) });
                let data = await res.json();
