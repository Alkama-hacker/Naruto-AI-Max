...
$db_user = 'user_fef03df8'; // সঠিক ইউজারনেম
$db_pass = 'pw_ed7bcd88';   // সঠিক পাসওয়ার্ড
...
// ==========================================
// 🚀 BACKEND API LOGIC (AJAX HANDLERS)
// ==========================================
if (isset($_GET['action']) || isset($_POST['action'])) {
    ...
    if (!$pdo) {
        echo json_encode(['error' => true, 'message' => 'Database Not Connected']);
        exit;
    }

    // 1. Sync & GAP FILLER (অফলাইন ডাটার জন্য হুবহু সার্ভারের রেজাল্ট বসানো)
    if ($action == 'sync') {
        $data = json_decode(file_get_contents('php://input'), true);
        if (is_array($data)) {
            // পুরাতন ডেটা থেকে নতুন ডেটার দিকে লুপ চালানোর জন্য রিভার্স
            ...
            foreach ($data as $item) {
                ...
                // যদি পিরিয়ডটি ডাটাবেসে না থাকে (মানে আপনি অফলাইনে ছিলেন)
                if (!$row) {
                    // গ্যাপ থাকা পিরিয়ডগুলোতে অরিজিনাল সার্ভারের রেজাল্টই প্রেডিকশন হিসেবে বসবে!
                    $pred = $actualSize; 
                    $status = 'W'; // যেহেতু প্রেডিকশন এবং একচুয়াল সেম, তাই ডিরেক্ট Win!

                    $ins = $pdo->prepare("INSERT INTO naruto_history (period, prediction, actual, status) VALUES (?, ?, ?, ?)");
                    $ins->execute([$period, $pred, $actualSize, $status]);
                } 
                // যদি পিরিয়ডটি থাকে কিন্তু রেজাল্ট 'P' (Pending) থাকে (মানে আপনি প্যানেলে অনলাইনে ছিলেন)
    ...
    // 2. Generate Prediction (লাইভ প্রেডিকশন লজিক)
    if ($action == 'get_prediction') {
        ...
        if ($row) {
            echo json_encode(['prediction' => $row['prediction'], 'pattern' => false]);
        } else {
            // Check Pattern for Live Prediction
...
<head>
    ...
    <title>NARUTO AI MAX - V5 PRO</title>
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', sans-serif; }
        ...
        body {
            ...
            background: radial-gradient(circle at center, rgba(0,255,120,0.08), transparent 35%),
                repeating-linear-gradient(0deg, rgba(0,255,120,0.035) 0px, rgba(0,255,120,0.035) 1px, transparent 1px, transparent 24px),
                #020402;
        }
        body::before {
        ...
        .timer-row {
            ...
            background: rgba(0, 0, 0, 0.8); border-radius: 8px; padding: 8px 10px;
            margin-bottom: 12px; border: 1px solid rgba(255, 255, 255, 0.05); box-shadow: inset 0 2px 10px rgba(0,0,0,1);
        }
        ...
        .login-box input {
            width: 100%; height: 36px; border: 1px solid rgba(0, 255, 102, 0.5); background: rgba(0,0,0,0.5);
            color: #8fff59; border-radius: 8px; padding: 0 12px; outline: none; margin-bottom: 12px; font-weight: bold; text-align: center;
        }
        .login-btn {
            width: 100%; height: 36px; border: 1px solid #00ff66; border-radius: 8px; background: rgba(0, 20, 0, 0.8);
            color: #84ff55; font-size: 13px; font-weight: 800; cursor: pointer; text-shadow: 0 0 6px rgba(132, 255, 85, 0.35);
        }
        ...
        .history-box {
            width: 90%; max-width: 450px; height: 80%; background: #080808; border: 1px solid #00ff66;
            border-radius: 12px; display: flex; flex-direction: column; overflow: hidden;
            box-shadow: 0 0 20px rgba(0,255,102,0.15); font-family: monospace;
        }
        .hist-header {
        ...
        .badge {
            ...
            width: 20px; height: 20px; border-radius: 50%; color: #fff; font-weight: 800; font-size: 10px;
        }
...
<body>
    ...
    <script>
        ...
        // ===================================
        // Fetch Last Periods from API (Auto Catch-Up)
        // ===================================
        async function fetchPeriod(callback) {
            try {
                let r = await fetch(API_URL, {
                    ...
                    // pageSize: 144 আপনার রিকোয়ারমেন্ট অনুযায়ী দেওয়া হয়েছে
                    body: JSON.stringify({ 
                        "pageSize": 144, 
                        "pageNo": 1, 
                        "typeId": 30, 
                        ...
                        "signature": "558BB707A61FA8D6BBB1748C5065F77A", 
                        "timestamp": 1779019083 
                    })
                });
                ...
                if (list && list.length > 0) {
                    ...
                    // এটি ব্যাকএন্ডে পাঠিয়ে দিবে, ব্যাকএন্ড নিজে নিজে গ্যাপগুলো (মিসিং ডেটা) অরিজিনাল রেজাল্ট দিয়ে পূরণ করবে
        ...
        async function generateAIResult() {
            try {
                ...
                if(data && data.prediction) {
                    renderUI(data.prediction, data.pattern);
                    return;
                }
            } catch(e) {}
            
            let isBig = Math.random() > 0.5;
            renderUI(isBig ? 'BIG' : 'SMALL', false);
        }
        ...
        function startSystem() {
            // Loading ইস্যু ফিক্স করার জন্য ফার্স্ট টাইম কল করা হচ্ছে
            fetchPeriod(function(){ generateAIResult(); }); 
            
            setInterval(() => {
                var rem = getCountdown();
                document.getElementById('timer-display').innerText = rem < 10 ? '0'+rem : rem;
        ...
        async function loadHistoryData() {
            ...
            try {
                let res = await fetch('?action=get_history');
                let data = await res.json();
                
                if(data.error) {
                    tbody.innerHTML = `<tr><td colspan="4" style="color:#ff1744; font-weight:bold;">${data.message}</td></tr>`;
                    return;
                }

                document.getElementById('totWin').innerText = data.wins || 0;
                document.getElementById('totLoss').innerText = data.losses || 0;
                ...
                if (data.data && data.data.length > 0) {
                    data.data.forEach(row => {
                        ...
                        tbody.innerHTML += `
                            <tr>
                                <td>${row.period}</td>
                                <td style="font-weight:bold;color:#fff;">${row.prediction}</td>
                                <td>${row.actual}</td>
                                <td><div class="badge ${badgeClass}">${row.status}</div></td>
                            </tr>
                        `;
                    });
                } else {
                ...
                tbody.innerHTML = '<tr><td colspan="4" style="color:red;">API Error!</td></tr>';
            }
        }
        ...
        async function deleteHistory() {
            ...
            if(pass) {
                let res = await fetch('?action=delete_history', {
                    method: 'POST',
                    headers: {'Content-Type': 'application/json'},
